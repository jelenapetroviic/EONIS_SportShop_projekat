...
// DELETE BANNER
const deletedBanned = asyncHandler(async (req, res) => {
  const banner = await Banner.findByIdAndDelete(req.params.id);
  if (!banner) {
    res.status(400);
    throw new Error("Banner was not deleted");
  } else {
    res.status(201).json("Banner was deleted saccessfully");
  }
});
...
export {getAllBanners, createBanner, deletedBanned, getRandomBanner}